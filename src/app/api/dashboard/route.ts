...
export async function GET() {
  try {
    ...
    const monthlyRevenue = paidThisMonth.reduce((s, i) => s + i.nominal, 0);
    ...
    const yearlyRevenue = paidYTD.reduce((s, i) => s + i.nominal, 0);
    ...
    const outstandingTotal = outstandingInv.reduce((s, i) => s + i.nominal, 0);
    const unpaidCount = outstandingInv.filter((i) => i.status !== 'paid').length;
    ...
    const wonDeals = allDealsYTD.filter((d) => d.dealStatus === 'won').length;
    ...
    for (const d of wonDealsWithService) {
    ...
    const maxSvc = Math.max(...Object.values(svcMap).map((s) => s.total), 1);
    ...
    const leaderboard = await Promise.all(aeUsers.map(async (u) => {
      const deals = await prisma.deal.findMany({
        where: { assignedAeId: u.id, dealStatus: 'won', tanggalMasuk: { gte: monthStart, lt: monthEnd } },
      });
      const rev = deals.reduce((s, d) => s + d.nilai, 0);
    ...
    leaderboard.sort((a, b) => b.rev - a.rev);
    ...
    const weightedTotal = activeDeals.reduce((s, d) => s + d.nilai * d.probability / 100, 0);
